...
class GatedFeatureFusion(nn.Module):
    ...
    def __init__(self, d_model: int = 256, gate_bias: float = 0.0) -> None:
        ...
        if gate_bias != 0:
            self.gate[-2].bias.data.fill_(gate_bias)  # type: ignore[operator]
...
class MAATModel(nn.Module):
    ...
    def get_pathway_importance(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        gates = torch.stack(
            result["gates"], dim=0  # type: ignore[arg-type]
        )  # [layers, batch, seq, d_model]  # type: ignore[arg-type]
...
class MAATLoss(nn.Module):
    ...
    def forward(
        ...
        if result.get("gates"):
            gates = torch.stack(result["gates"], dim=0)  # type: ignore[arg-type]
