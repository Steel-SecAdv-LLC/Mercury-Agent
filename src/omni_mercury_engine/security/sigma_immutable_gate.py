...
class SigmaImmutableGate:
    ...
    **Authority and known limitation (read before trusting the score).**
    The learned network is trained on a SYNTHETIC corpus: scalar vectors
    generated by ``scripts/train_sigma_immutable.py`` and labelled by a
    threshold rule over the ETHICAL-group scalars — not by real-world
    ethical outcomes.  Measured consequence: on its own, the trained
    network passed vectors with a single critical ethical dimension
    zeroed (e.g. benevolence -> 0).  The **authoritative** ethical gate is
    therefore the deterministic critical-ethical floor
    (:meth:`enforce_ethical_floor`), which callers compose BEFORE
    :meth:`enforce` (see ``OmniMercuryEngine.detect_with_fusion``): a
    collapsed anchor is a categorical, fail-closed refusal no learned
    score can override.  The learned score is a secondary, advisory
    check for non-obvious multi-dimensional correlations; retraining it
    on a real labelled ethical-outcome corpus is tracked in
    ``docs/DORMANCY_LEDGER.md``.
    """
