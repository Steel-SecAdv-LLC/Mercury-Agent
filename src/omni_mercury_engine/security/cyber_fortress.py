...
class CyberFortress(LoggerMixin):
    ...
    def fortress_scan(self, system_data: dict[str, Any]) -> FortressResult:
        ...
        if self.enable_hash_integrity and self.hash_checker is not None and "hash_chain" in system_data:
        ...
        if self.enable_zero_day_sim and self.zero_day_sim is not None and "system_state" in system_data:
        ...
        if self.enable_traffic_detection and self.traffic_detector is not None and "network_traffic" in system_data:
