    ...
    def reach_consensus(
        ...
        if len(det_results) < self.min_participants:
            return ConsensusResult(
                ...
                confidence=0.0,
                agreement_ratio=0.0,
                participant_count=len(det_results),
                method_used=self.method,
            )

        if self.method == ConsensusMethod.MAJORITY_VOTE:
            return self._majority_vote(consensus_id, det_results)
        elif self.method == ConsensusMethod.WEIGHTED_VOTE:
            return self._weighted_vote(consensus_id, det_results)
        elif self.method == ConsensusMethod.UNANIMOUS:
            return self._unanimous(consensus_id, det_results)
        elif self.method == ConsensusMethod.BYZANTINE_TOLERANT:
            return self._byzantine_tolerant(consensus_id, det_results)
        elif self.method == ConsensusMethod.CONFIDENCE_WEIGHTED:
            return self._confidence_weighted(consensus_id, det_results)
        else:
            return self._majority_vote(consensus_id, det_results)
