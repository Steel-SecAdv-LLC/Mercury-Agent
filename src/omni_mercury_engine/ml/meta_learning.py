...
class Task:
    ...
    task_id: str
    task_name: str
    support_set: list[tuple[np.ndarray[Any, Any], int]]  # (features, label) pairs
    query_set: list[tuple[np.ndarray[Any, Any], int]]
    n_way: int = 2  # Normal vs anomaly
    k_shot: int = 5
    domain: str = "general"
    metadata: dict[str, Any] = field(default_factory=dict)

    def get_support_arrays(self) -> tuple[np.ndarray[Any, Any], np.ndarray[Any, Any]]:
        """Get support set as arrays."""
        X = np.array([x for x, _ in self.support_set])
        y = np.array([y for _, y in self.support_set])
        return X, y

    def get_query_arrays(self) -> tuple[np.ndarray[Any, Any], np.ndarray[Any, Any]]:
...
class PrototypicalNetworks:
    ...
    def compute_prototypes(
        ...
        if isinstance(support_set, dict):
            self._class_names = {}
            self._name_to_id = {}
            class_embeddings: dict[int, list[np.ndarray[Any, Any]]] = defaultdict(list)
        ...
        class_embeddings_list: dict[int, list[np.ndarray[Any, Any]]] = defaultdict(list)
...
class MAML:
    ...
    def meta_train_step(
        ...
        Returns:
            Average meta-loss
        """
        meta_gradients: dict[str, np.ndarray[Any, Any]] = {
...
class MetaLearningAdapter:
    ...
    def create_task(
        self,
        support_data: list[tuple[np.ndarray[Any, Any], int]],
        query_data: list[tuple[np.ndarray[Any, Any], int]],
    ...
    def generate_episodes(
        self,
        data: np.ndarray[Any, Any] | dict[str, np.ndarray[Any, Any]],
    ...
    def evaluate_n_way_k_shot(
        self,
        data_or_episodes: dict[str, np.ndarray[Any, Any]] | list[dict[str, Any]],
        n_way: int | None = None,
        k_shot: int | None = None,
        n_episodes: int = 10,
    ) -> float:
...
class AnomalyMetaLearner:
    ...
    def __init__(
        ...
        self._feature_importance: np.ndarray[Any, Any] | None = None

        # Online adaptation buffer
        self._online_buffer: list[tuple[np.ndarray[Any, Any], bool]] = []

    def learn_new_type(
        self,
        type_name_or_data: str | dict[str, np.ndarray[Any, Any]],
        ...
        examples_arr: np.ndarray[Any, Any] = examples
    ...
    def adapt_to_new_anomaly_type(
        self,
        normal_examples: list[np.ndarray[Any, Any]],
