...
class NativeOneClassSVM:
    """One-Class SVM using RBF kernel approximation.

    Uses random Fourier features to approximate the RBF kernel and then
    fits a linear decision boundary in the lifted space.
    """
    ...
    def fit(self, X: np.ndarray) -> NativeOneClassSVM:
        X = np.asarray(X, dtype=np.float64)
        n, d = X.shape

        # Compute gamma
        if self.gamma == "scale":
            self._gamma_val = 1.0 / (d * X.var() + 1e-12)
        elif self.gamma == "auto":
            self._gamma_val = 1.0 / (d + 1e-12)
        else:
            self._gamma_val = float(self.gamma)

        # Random Fourier features for RBF kernel approximation
        self._w = self.rng.randn(d, self.n_features_rff) * np.sqrt(
            2 * self._gamma_val
        )
        self._b = self.rng.uniform(0, 2 * np.pi, self.n_features_rff)

        Z = self._transform(X)
        # Compute centroid in feature space
        self._center = Z.mean(axis=0)
        # Distances from centroid
        dists = np.linalg.norm(Z - self._center, axis=1)
        # Set threshold at (1 - nu) quantile
        self._offset = float(np.percentile(dists, 100 * (1 - self.nu)))
        return self

    def _transform(self, X: np.ndarray) -> np.ndarray:
        assert self._w is not None and self._b is not None
        Z = np.cos(X @ self._w + self._b) * np.sqrt(2.0 / self.n_features_rff)
        return Z
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        Z = self._transform(X)
        dists = np.linalg.norm(Z - self._center, axis=1)
        return np.where(dists > self._offset, -1, 1)
    ...
    def decision_function(self, X: np.ndarray) -> np.ndarray:
        ...
        Z = self._transform(X)
        dists = np.linalg.norm(Z - self._center, axis=1)
        return self._offset - dists


class NativeLocalOutlierFactor:
    """Local Outlier Factor using brute-force k-NN.

    Novelty detection mode: fit on training data, predict on new data.
    """
    ...
    def fit(self, X: np.ndarray) -> NativeLocalOutlierFactor:
        from scipy.spatial.distance import cdist
        ...
        sorted_idx = np.argsort(dists, axis=1)[:, :k]
        k_dists = np.take_along_axis(dists, sorted_idx, axis=1)
        k_dist = k_dists[:, -1]  # k-th distance

        # Reachability distance
        reach_dists = np.maximum(k_dists, k_dist[sorted_idx])

        # Local reachability density
        self._lrd_train = 1.0 / (reach_dists.mean(axis=1) + 1e-12)

        # LOF scores for training data (for threshold)
        lof_scores = np.zeros(n)
        for i in range(n):
            neighbors = sorted_idx[i]
            lof_scores[i] = self._lrd_train[neighbors].mean() / (
                self._lrd_train[i] + 1e-12
            )

        self._threshold = float(np.percentile(lof_scores, 100 * (1 - self.contamination)))
        return self
    ...
    def decision_function(self, X: np.ndarray) -> np.ndarray:
        """Negative LOF score (lower = more anomalous)."""
        from scipy.spatial.distance import cdist

        assert self._X_train is not None and self._lrd_train is not None
        X = np.asarray(X, dtype=np.float64)
        k = min(self.n_neighbors, len(self._X_train) - 1)

        dists = cdist(X, self._X_train)
        sorted_idx = np.argsort(dists, axis=1)[:, :k]
        k_dists = np.take_along_axis(dists, sorted_idx, axis=1)

        # k-distance of training neighbors
        train_dists = cdist(self._X_train, self._X_train)
        np.fill_diagonal(train_dists, np.inf)
        train_sorted = np.argsort(train_dists, axis=1)[:, :k]
        train_k_dist = np.take_along_axis(train_dists, train_sorted, axis=1)[:, -1]

        reach_dists = np.maximum(k_dists, train_k_dist[sorted_idx])
        lrd_new = 1.0 / (reach_dists.mean(axis=1) + 1e-12)

        lof_scores = np.zeros(len(X))
        for i in range(len(X)):
            neighbors = sorted_idx[i]
            lof_scores[i] = self._lrd_train[neighbors].mean() / (lrd_new[i] + 1e-12)

        return -lof_scores


class NativeEllipticEnvelope:
    """Robust outlier detection using Minimum Covariance Determinant (MCD).

    Simplified: uses empirical covariance with regularization, then
    Mahalanobis distance for outlier scoring.
    """
    ...
    def fit(self, X: np.ndarray) -> NativeEllipticEnvelope:
        X = np.asarray(X, dtype=np.float64)
        n, d = X.shape

        if self.support_fraction is not None:
            # Use a fraction of the data for robust estimation
        ...
        self._mean = X_support.mean(axis=0)
        cov = np.cov(X_support, rowvar=False)
        if cov.ndim == 0:
        ...
        # Mahalanobis distances on training data
        mahal = self._mahalanobis(X)
        self._threshold = float(
            np.percentile(mahal, 100 * (1 - self.contamination))
        )
        return self
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        X = np.asarray(X, dtype=np.float64)
        mahal = self._mahalanobis(X)
        return np.where(mahal > self._threshold, -1, 1)

    def decision_function(self, X: np.ndarray) -> np.ndarray:
        """Signed distance to decision boundary (negative = anomaly)."""
        X = np.asarray(X, dtype=np.float64)
        mahal = self._mahalanobis(X)
        return self._threshold - mahal
