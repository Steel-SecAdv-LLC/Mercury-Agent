...
class TemporalSequenceEncoder(nn.Module):
    ...
    def __init__(
        ...
        if mode == "lstm":
            ...
            self.rnn = nn.GRU(  # type: ignore[assignment, unused-ignore]
                input_dim,
                hidden_dim,
                num_layers=num_layers,
                batch_first=True,
    ...
    def get_temporal_features(self, x: torch.Tensor, pool: str = "last") -> torch.Tensor:
        ...
        if pool == "last":
            ...
            return self.forward(x, return_sequence=False)  # type: ignore[return-value, unused-ignore]
        else:
...
class OmniFusionModel(nn.Module):
    ...
    def get_detector_importance(
        ...
        with torch.no_grad():
            ...
            weights = output["attention_weights"]["detector_weights"]  # type: ignore[index, unused-ignore]
    ...
    def train_with_optimizers(
        ...
        optimizers for decoupled (lock-free) training updates with a Lyapunov-style
        decay-schedule stability monitor (measured, not a guarantee).
        ...
        if len(lyapunov_values) > 10:
            ...
            lyapunov_stable = np.mean(lyapunov_diff) <= lambda_lyapunov * 0.1  # type: ignore[assignment, unused-ignore]
