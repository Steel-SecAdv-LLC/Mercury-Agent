    ...
    def fit(
        ...
        for _epoch in range(epochs):
            for i in range(0, len(X_train), batch_size):
                ...
                losses["total_loss"].backward()  # type: ignore[no-untyped-call, unused-ignore]
                self.optimizer.step()

        self.vae.eval()
        with torch.no_grad():
            train_scores = self.vae.anomaly_score(X_train)
            self.threshold = train_scores.mean() + 3 * train_scores.std()  # type: ignore[assignment, unused-ignore]
