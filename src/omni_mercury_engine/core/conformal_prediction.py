...
class CrossConformalPredictor:
    ...
    def fit(
        ...
        all_scores = []  # type: ignore[var-annotated, unused-ignore]
...
class ConformalAnomalyDetector:
    ...
    def _get_anomaly_scores(self, X: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if proba.ndim == 2:
                return proba[:, 1]  # type: ignore[no-any-return, unused-ignore]
            return proba  # type: ignore[no-any-return, unused-ignore]
        ...
        try:
            ...
            return 1.0 / (1.0 + np.exp(-decision))  # type: ignore[no-any-return, unused-ignore]
        except (AttributeError, NotImplementedError):
            pass
        ...
        try:
            ...
            if max_score > min_score:
                return 1.0 - (scores - min_score) / (max_score - min_score)  # type: ignore[no-any-return, unused-ignore]
    ...
    def _compute_ensemble_anomaly_scores(self, X: np.ndarray) -> np.ndarray:
        ...
        return np.clip(ensemble_scores, 0.0, 1.0)  # type: ignore[no-any-return, unused-ignore]
...
def _extract_detector_scores(detector: Any, X: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        if max_s > min_s:
            return 1.0 - (scores - min_s) / (max_s - min_s)  # type: ignore[no-any-return, unused-ignore]
        return np.full(len(scores), 0.5)
    except (AttributeError, NotImplementedError):
        pass
