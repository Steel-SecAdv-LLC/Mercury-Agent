...
class AdaptiveNoiseFilter:
    ...
    def _soft_threshold(self, data: np.ndarray[Any, Any], threshold: float) -> np.ndarray[Any, Any]:
        ...
        return np.sign(data) * np.maximum(np.abs(data) - threshold, 0)  # type: ignore[no-any-return, unused-ignore]
    ...
    def _savitzky_golay(self, data: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
        ...
        try:
            return signal.savgol_filter(data, window, poly_order)  # type: ignore[no-any-return, unused-ignore]
        except ValueError:
            return data.copy()
    ...
    def _adaptive_bandpass(self, data: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
        ...
        try:
            ...
            return signal.filtfilt(b, a, data)  # type: ignore[no-any-return, unused-ignore]
        except ValueError:
            return data.copy()
    ...
    def _median_filter(self, data: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
        ...
        """
        return signal.medfilt(data, kernel_size=min(self.config.window_size, len(data)))  # type: ignore[no-any-return, unused-ignore]
