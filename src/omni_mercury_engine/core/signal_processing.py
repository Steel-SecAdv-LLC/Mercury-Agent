...
class AdaptiveNoiseFilter:
    ...
    def _soft_threshold(self, data: np.ndarray[Any, Any], threshold: float) -> np.ndarray[Any, Any]:
        ...
        return np.asarray(np.sign(data) * np.maximum(np.abs(data) - threshold, 0))
    ...
    def _savitzky_golay(self, data: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
        ...
        try:
            return np.asarray(signal.savgol_filter(data, window, poly_order))
        except ValueError:
            return data.copy()
    ...
    def _median_filter(self, data: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
        ...
        """
        return np.asarray(signal.medfilt(data, kernel_size=min(self.config.window_size, len(data))))
