...
if TORCH_AVAILABLE:
    ...
    class UncertaintyQuantifier(nn.Module):
        ...
        def forward(
            ...
            mc_samples = torch.stack(  # type: ignore[assignment, unused-ignore]
                mc_samples, dim=0
            )  # [n_samples, batch, embed_dim]

            # Epistemic uncertainty (model uncertainty from MC dropout)
            mean = mc_samples.mean(dim=0)  # type: ignore[attr-defined, unused-ignore]
            epistemic_var = mc_samples.var(dim=0).mean().item()  # type: ignore[attr-defined, unused-ignore]
            ...
            # Total uncertainty
            total_std = torch.sqrt(mc_samples.var(dim=0) + aleatoric_var.unsqueeze(-1))  # type: ignore[attr-defined, unused-ignore]
