...
class EnhancedBaseDetector:
    ...
    def _get_scores(self, X: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
        ...
        if hasattr(self.base_detector, "detect"):
            result = self.base_detector.detect(X)
            return np.asarray(result.get("scores", np.zeros(len(X))))  # type: ignore[no-any-return, unused-ignore]
        elif hasattr(self.base_detector, "predict"):
            result = self.base_detector.predict(X)
            if isinstance(result, dict):
                return np.asarray(result.get("anomaly_scores", np.zeros(len(X))))  # type: ignore[no-any-return, unused-ignore]
