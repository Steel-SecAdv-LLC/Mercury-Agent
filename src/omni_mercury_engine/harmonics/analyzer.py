...
class AdvancedHarmonicAnalyzer:
    ...
    def fit(
        ...
        if all_power_spectra:
            ...
            self._mean_complexity = float(np.mean(all_complexities))
            self._std_complexity = float(np.std(all_complexities)) + 1e-10
    ...
    def detect_anomalies(
        ...
        if self._reference_db and len(self._reference_db) > 0:
            ...
            if nearest:
                similarity_score = float(np.mean([s for _, s in nearest]))
