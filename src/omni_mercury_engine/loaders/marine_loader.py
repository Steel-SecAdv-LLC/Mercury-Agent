...
class MarineLoader(BaseDomainLoader):
    ...
    **PCA validation**: Top-3 PCA on the original 8-feature set yields
    ...
    def get_ground_truth(self, event_id: str) -> np.ndarray:
        ...
        richness dropped by more than 70% from the baseline are labeled
        ...
        for i, cell in enumerate(all_cells):
            ...
            # Require meaningful baseline (>=5 species) to label as
            # anomalous.  Cells with sparse baselines lack sufficient
            # diversity to reliably detect a decline.
            if baseline_val >= 5:
    ...
    def engineer_features(self, raw_data: pd.DataFrame) -> np.ndarray:
        ...
        baseline_richness_map: dict[str, float] = {}
        baseline_depth_map: dict[str, float] = {}
