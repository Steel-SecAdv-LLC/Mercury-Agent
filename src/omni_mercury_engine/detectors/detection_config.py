...
2. **A single, consistent magnitude regime.**  All caps/clamps in the tier use
   ...
   ``1e100``).  This is the *same* bound the tier's shared ``_calibration.bound_finite``
   input sanitiser uses (``_calibration.FINITE_CAP`` is defined as this constant),
   so the two are unified on one number rather than each picking its own literal.
   ``1e100`` (not the API layer's ``1e15``) is chosen deliberately: it is
   astronomically larger than any physical/operational signal Mercury ingests
   (nanosecond-epoch timestamps ~1.7e18, financial magnitudes ~1e12) so realistic
   data is *never* clipped, while its square (``1e200``) stays comfortably within
   float64 range so the FFT/covariance/Gram arithmetic that overflowed at
   ``np.nan_to_num``'s ``1.8e308`` sentinel no longer can.  ``±inf`` maps to
   ``±max_magnitude`` and every finite value is clamped into
   ``[-max_magnitude, max_magnitude]`` so no detector can emit a value outside a
   known, finite envelope.
...
#: Single documented safe magnitude bound for the whole tier -- ``1e100``. This is
#: the one number both this policy layer and the tier's shared
#: ``_calibration.bound_finite`` input sanitiser clamp to (``_calibration.FINITE_CAP``
#: is defined as this constant), so the magnitude regime is unified on one literal.
#: ``1e100`` is far larger than any realistic signal (so real data is never
#: clipped) yet its square stays inside float64 range (so the downstream
#: FFT/covariance/Gram arithmetic that overflowed ``np.nan_to_num``'s ``1.8e308``
#: sentinel cannot).
DEFAULT_MAX_MAGNITUDE: float = 1e100
...
def active_nan_policy() -> NaNPolicy:
    """The tier's active default NaN policy from the environment.

    Reads ``OMNI_DETECTOR_NAN_POLICY`` (default ``neutral``) directly -- a cheap
    read for the shared ``_calibration`` guards' hot path, so they honour the
...
def record_nonfinite_correction(
    detector: str, policy: NaNPolicy | str, field: str, *, n_nan: int, n_inf: int
) -> None:
    """Record (counter + structured log) that a guard rescued non-finite values.

    The public entry point the shared ``_calibration`` guards (``bound_finite`` /
    ``finite_scores`` / ``finite_features``) call so they emit the *same*
    ``omni_detector_nonfinite_corrected`` metric and structured warning as
    :func:`apply_nan_policy` / :func:`guard_finite_scalar`.
    """
    policy = NaNPolicy.coerce(policy)
    total = int(n_nan) + int(n_inf)
    if total <= 0:
        return
    _record_correction(detector, policy, field, total)
    logger.warning(
