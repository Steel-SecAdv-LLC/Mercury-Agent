...
class CorrelationAwareDecorrelator:
    ...
    def calibrate(
        ...
        for cluster in clusters:
            ...
            best_local_idx = max(
                cluster,
                key=lambda idx: fit_qualities.get(
                    probe_names[valid_indices[idx]], 0.0
                ),
            )
...
class PhiWeightedFusion:
    ...
    def fuse(
        ...
        confidences = np.array(
            [r.confidence for r in probe_results], dtype=np.float64
        )
        ...
        if decorrelator is not None and decorrelator.is_calibrated:
            ...
            logger.debug(
                "Decorrelator not calibrated; proceeding with "
                "unmodified confidence weights."
            )
        ...
        score_matrix = np.column_stack(
            [r.deviation_scores[:min_len] for r in probe_results]
        )
