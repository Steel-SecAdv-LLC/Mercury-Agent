...
class PhononInteractionNetwork(nn.Module):
    ...
    def forward(
        ...
        off_diagonal_mean = (
            coupling_matrix[~torch.eye(num_active_modes, dtype=bool, device=mode_amplitudes.device)]  # type: ignore[call-overload]
            .abs()
            .mean()
        )
    ...
    def _compute_schumann_alignment(
        ...
        for schumann_freq in SCHUMANN_HARMONICS:
            ...
            if idx > 0 and idx < len(spectrum) - 1:
                ...
                local_mean = np.mean(spectrum[max(0, idx - 5) : min(len(spectrum), idx + 6)])  # type: ignore[misc]
        ...
        weights = np.array(weights) / sum(weights)  # type: ignore[assignment]
