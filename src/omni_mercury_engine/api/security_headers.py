...
"""Security response headers for every response the middleware stack returns.

One documented exclusion: a response generated by Starlette's
``ServerErrorMiddleware`` — the 500 produced when a route raises an unhandled
exception — carries none of these headers. That middleware sits *outside* the
user middleware stack, so ``call_next`` re-raises and this middleware never
sees a response to decorate. Measured, not assumed: a route raising
``RuntimeError`` answers 500 with no ``Content-Security-Policy`` and no
``X-Frame-Options``, while every handled response (including 404s and the
middleware-generated 429/503) carries the full set. It is not fixable from
this layer; an application that needs headers on unhandled 500s has to install
a custom ``exception_handler`` or terminate them at the edge.
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    """Attach the security header set to every response that reaches it.

    Registered outermost of the *user* middleware stack, so it also covers
    responses short-circuited by the inner middleware — the rate limiter's 429
    and the quota layer's 503 never reach a route handler, and both are as
    much a browser-reachable response as a 200.

    "Outermost of the user stack" is not "outermost": Starlette's
    ``ServerErrorMiddleware`` wraps this one, so the 500 it synthesises for an
    unhandled exception is emitted without these headers (see the module
    docstring).
