...
ROSTER: tuple[RosterEntry, ...] = (
    ...
    RosterEntry(
        ...
        # ``utils`` carries the real ``compress_information`` operator entrypoint;
        # ``crypto`` seals the archived blob with an integrity digest.
        subsystems=("data", "crypto", "utils"),
        anchor="OMNI_UNIVERSAL",
    ...
    RosterEntry(
        ...
        # ``utils`` carries the real ``normalize_data`` operator entrypoint (the
        # ``data`` package is a deprecation shim that forwards to it).
        subsystems=("utils", "data", "models"),
        anchor="OMNI_UNIVERSAL",
        keywords=(
            "normalization",
            "canonicalization",
