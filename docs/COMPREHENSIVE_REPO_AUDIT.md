...
3. ✅ **Remove `continue-on-error` from security CI steps** - Removed from Semgrep, ethics audit, and Trivy Docker scan; Trivy narrowed to CRITICAL/HIGH with `exit-code: 1` and is the **enforcing** dep-CVE gate (mirrored by the standalone `Filesystem Security Scan` job in `security.yml`); ethics audit now returns exit code 1 on unexpected failures.  Safety and pip-audit remain **informational** (Safety v3.x's `safety check` has a known schema-validation bug that prevents v3 policy files from filtering accepted CVEs — `.safety-policy.yml` is still passed via `--policy-file` so the documented risk acceptances are in scope) but were upgraded from `|| true` (which silently turns vulnerability findings into a green ✓) to **`continue-on-error: true`** so that findings now surface as a red ❌ on the job summary while still allowing the PR to proceed.  This makes regressions visible to maintainers without conflating informational with enforcing.  Findings continue to upload as JSON artifacts (`safety-report.json`, `pip-audit-report.json`) for review *(branch: `claude/improve-previous-work-k2tWf`)*
...
- Trivy on the built Docker image (and the standalone `Filesystem Security Scan`) hard-fail on CRITICAL/HIGH dep CVEs; Safety and pip-audit run informationally via `continue-on-error: true` (so findings show as red ❌ on the job summary instead of being silently masked behind a green ✓ as the previous `|| true` did) with results uploaded as artifacts (Safety v3 schema bug prevents true policy-file enforcement at present)
...
- **CI security** - ~~all checks are soft-fail~~ `continue-on-error` removed from Semgrep, ethics, Trivy (Docker + Filesystem); `.safety-policy.yml` documents accepted CVEs and is passed via `--policy-file` to Safety; Safety + pip-audit upgraded from `|| true` (silent green ✓ on findings) to `continue-on-error: true` (visible red ❌ on findings, workflow continues so the PR isn't blocked); **remaining**: Safety + pip-audit are still informational until the Safety v3 schema bug is upstream-fixed — Trivy is the enforcing dep-CVE gate; pydocstyle and docs build are advisory by design
