...
3. ✅ **Remove `continue-on-error` / `|| true` from security CI steps** - Removed from Semgrep, ethics audit, Bandit, and Trivy Docker scan; Trivy narrowed to CRITICAL/HIGH with `exit-code: 1` and is the **enforcing** dep-CVE gate (mirrored by the standalone `Filesystem Security Scan` job in `security.yml`); ethics audit now returns exit code 1 on unexpected failures.  **Semgrep is now BLOCKING** with hardened install (missing semgrep `exit 1`s rather than silently passing).  Safety and pip-audit are intentionally kept at **`continue-on-error: true`** (strict improvement over the previous `|| true` — findings now surface as a red ❌ on the GHA UI instead of a green ✓), because Safety v3.x's `safety check` command only accepts legacy v2 policy files (`.safety-policy.yml` is in the modern v3 format), and Safety v3's v3-aware `safety scan` command requires Safety Cloud authentication that cannot be provisioned in CI without a baked credential.  Without a working accept-list mechanism, full blocking would fail every PR on baseline transitive Python-package CVEs outside the PR's scope.  Trivy remains the enforcing gate (with `.trivyignore`).  Findings upload as JSON artifacts (`safety-report.json`, `pip-audit-report.json`) for triage.  *(branch: `claude/improve-previous-work-k2tWf`)*
...
- Trivy on the built Docker image (and the standalone `Filesystem Security Scan`) hard-fail on CRITICAL/HIGH dep CVEs and honor `.trivyignore` for documented risk acceptances; Semgrep (SAST) and Bandit are blocking; Safety and pip-audit are informational via `continue-on-error: true` (findings surface as red ❌ rather than silent green ✓) until a working accept-list mechanism is available — Safety `check` rejects the v3 policy file and Safety `scan` requires Safety Cloud auth.  Results upload as JSON artifacts for triage
...
- **CI security** - ~~all checks are soft-fail~~ Semgrep, Bandit, Ethics Audit, and Trivy (Docker + Filesystem) are now blocking with no escape hatches; Semgrep install hardened so missing semgrep `exit 1`s rather than silently skipping.  Safety and pip-audit are kept at `continue-on-error: true` (strict improvement over the previous `|| true` — visible red ❌ instead of silent green ✓) until either Safety `check` supports v3 policy files OR Safety `scan` gains a credential-free CI mode; the v3 `.safety-policy.yml` documenting accepted CVEs is rejected by Safety `check`'s legacy-only parser, and Safety `scan` requires Safety Cloud auth that cannot be provisioned in CI without baking a credential.  Risk-accepted CVEs are honored via `.trivyignore` in the parallel Trivy gates, which remain the enforcing dep-CVE hard-fail.  A new **Neuro-Symbolic Tests** job runs on every PR and is wired into the `CI Success` rollup, gating the 7-phase cognitive architecture (`tests/cognitive/`, `tests/safeguards/`, neurosymbolic/3R/ethics suites) so cognitive regressions cannot land on feature branches.  `pydocstyle` and Sphinx docs build remain advisory by design (their failures are codebase-wide hygiene work, separate from this PR's scope)
