...
3. ✅ **Remove `continue-on-error` from security CI steps** - Removed from Semgrep, ethics audit, Bandit, Safety, pip-audit, and Trivy Docker scan; Trivy narrowed to CRITICAL/HIGH with `exit-code: 1` and is the **enforcing** dep-CVE gate (mirrored by the standalone `Filesystem Security Scan` job in `security.yml`); ethics audit now returns exit code 1 on unexpected failures.  **Safety and pip-audit are now BLOCKING** — known dependency CVEs and PyPI/OSV findings hard-fail the PR.  The Safety v3.x `--policy-file` schema bug is worked around by NOT passing the policy file (Safety would silently crash); risk acceptances are honored consistently via `.trivyignore` in the parallel Trivy gates.  Findings continue to upload as JSON artifacts (`safety-report.json`, `pip-audit-report.json`) for triage.  Semgrep installation is also hardened: missing semgrep now `exit 1`s rather than silently passing the gate.  *(branch: `claude/improve-previous-work-k2tWf`)*
...
- Trivy on the built Docker image (and the standalone `Filesystem Security Scan`) hard-fail on CRITICAL/HIGH dep CVEs; Safety and pip-audit are now also blocking (no `continue-on-error`) — known CVEs / PyPI advisories fail the PR.  Risk-accepted CVEs are honored via `.trivyignore` in the parallel Trivy gates (Safety v3 schema bug prevents using `--policy-file` directly).  Results upload as JSON artifacts for triage
...
- **CI security** - ~~all checks are soft-fail~~ `continue-on-error` removed from every security/ethics step (Semgrep, Bandit, Safety, pip-audit, Ethics Audit, Trivy Docker + Filesystem).  Safety and pip-audit are now FULLY BLOCKING — known dependency CVEs and PyPI advisories hard-fail the PR rather than just surfacing as red ❌; risk-accepted CVEs are honored via `.trivyignore` in the parallel Trivy gates (the Safety v3.x `--policy-file` schema bug means we don't pass the policy file directly).  Semgrep install hardened: missing semgrep now `exit 1`s rather than silently skipping.  A new **Neuro-Symbolic Tests** job runs on every PR and is wired into the `CI Success` rollup, gating the 7-phase cognitive architecture (`tests/cognitive/`, `tests/safeguards/`, neurosymbolic/3R/ethics suites) so cognitive regressions cannot land on feature branches.  `pydocstyle` and Sphinx docs build remain advisory by design (their failures are codebase-wide hygiene work, separate from this PR's scope)
