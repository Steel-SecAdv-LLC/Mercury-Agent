...
- Trivy on the built Docker image (and the standalone `Filesystem Security Scan`) hard-fail on CRITICAL/HIGH dep CVEs; Safety and pip-audit run informationally with results uploaded as artifacts (Safety v3 schema bug prevents true policy-file enforcement at present)
...
- **CI security** - ~~all checks are soft-fail~~ `continue-on-error` removed from Semgrep, ethics, Trivy (Docker + Filesystem); `.safety-policy.yml` documents accepted CVEs and is passed via `--policy-file` to Safety; **remaining**: Safety + pip-audit are informational (`|| true` retained) until the Safety v3 schema bug is upstream-fixed — Trivy is the enforcing dep-CVE gate; pydocstyle and docs build are advisory by design
