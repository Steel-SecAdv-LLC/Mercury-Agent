# OMNI-AVA Runbook: PVC Near Capacity

## Alert: OmniAvaPVCNearCapacity

### Overview
This runbook provides guidance for responding when Persistent Volume Claims (PVCs) in the OMNI-AVA namespace are approaching storage capacity limits.
...
kubectl exec -n omni-ava deployment/omni-ava-engine -- df -h /data
...
kubectl exec -n omni-ava deployment/omni-ava-engine -- du -sh /data/*

# Find largest files
kubectl exec -n omni-ava deployment/omni-ava-engine -- find /data -type f -size +100M -exec ls -lh {} \;
```

### 3. Check Log Files
```bash
# Check log file sizes
kubectl exec -n omni-ava deployment/omni-ava-api -- du -sh /var/log/*

# Check for log rotation
kubectl exec -n omni-ava deployment/omni-ava-api -- ls -la /var/log/
```

### 4. Check Model Storage
```bash
# Check model directory size
kubectl exec -n omni-ava deployment/omni-ava-engine -- du -sh /data/models/*
...
1. Clean old log files:
   ```bash
   kubectl exec -n omni-ava deployment/omni-ava-api -- find /var/log -name "*.log" -mtime +7 -delete
   ```
2. Enable log rotation:
   ```bash
   kubectl set env deployment/omni-ava-api -n omni-ava LOG_ROTATION_ENABLED=true LOG_MAX_SIZE=100M LOG_MAX_FILES=5
   ```
...
1. Clean old model versions:
   ```bash
   kubectl exec -n omni-ava deployment/omni-ava-engine -- find /data/models -name "*.old" -delete
   ```
2. Enable model cache cleanup:
   ```bash
   kubectl set env deployment/omni-ava-engine -n omni-ava MODEL_CACHE_MAX_SIZE=10G MODEL_CACHE_CLEANUP_ENABLED=true
   ```
3. Remove unused models:
   ```bash
   kubectl exec -n omni-ava deployment/omni-ava-engine -- rm -rf /data/models/unused_model
   ```
...
1. Clean temporary files:
   ```bash
   kubectl exec -n omni-ava deployment/omni-ava-engine -- rm -rf /tmp/*
   kubectl exec -n omni-ava deployment/omni-ava-engine -- rm -rf /data/tmp/*
   ```
2. Enable automatic temp cleanup:
   ```bash
   kubectl set env deployment/omni-ava-engine -n omni-ava TEMP_CLEANUP_INTERVAL=1h
   ```
...
1. Expand PVC (if storage class supports it):
   ```bash
   kubectl patch pvc omni-ava-data -n omni-ava -p '{"spec":{"resources":{"requests":{"storage":"100Gi"}}}}'
   ```
