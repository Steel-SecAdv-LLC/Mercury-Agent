...
| `AMA_REQUIRE_CONSTANT_TIME` | unset | **Deprecated no-op with the pinned AMA v3.3.0.** AMA now enforces native-only operation unconditionally (INVARIANT-7), so its constant-time C primitives are always used and this variable has no effect (AMA logs a removal notice if set). The mandatory import-time PQC gate already guarantees the native backend is loaded, so `require_constant_time()`'s "AMA unavailable" branch is unreachable at runtime. Retained only to keep legacy workflows readable. |
