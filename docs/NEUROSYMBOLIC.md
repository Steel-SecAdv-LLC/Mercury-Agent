...
| **Legacy `LogicTensorNetwork`** (`models/neurosymbolic.py`) | a never-trained `nn.Module` whose random-init forward fed the fusion consensus as if it were a neural confidence | **RETIRED (2026-06-02)** | The untrained class is removed. `NeurosymbolicEngine.neural_inference` now returns a deterministic statistical heuristic (bounded, feature-responsive, reproducible) — no untrained network masquerading as trained. Canonical *trained* neuro-symbolic remains `SymbolicConstraintModule`. Regression: `tests/test_neurosymbolic_integration.py::{test_untrained_ltn_class_is_retired, test_neural_inference_is_deterministic_and_feature_responsive}`. |
