...
| `core/global_omni_scalar_network.py` `MultiHeadAttentionFusion` | "32-head attention" learned fusion | **Trained and merit-gated (2026-07-14).** `scripts/train_gosnn_fusion.py` harvests the production `fuse()` inputs from a real engine detecting on real ADBench windows and trains masked-member reconstruction candidates; the shipped `gosnn_attention_fusion` checkpoint beat the phi-weighted reference on held-out production state lists (gate: MSE <= 0.95x reference + anti-collapse; `artifacts/gosnn_fusion.eval.json`). Default construction serves the shipped winner; `load_shipped_weights=False` pins the deterministic reference, and the historical random-projection inference remains unreachable (`tests/core/test_attention_fusion_gate.py`). |
