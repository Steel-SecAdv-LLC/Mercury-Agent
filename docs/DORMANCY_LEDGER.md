...
| `core/global_omni_scalar_network.py` (~82 ISO-25010/Halstead/DORA/SLSA/… scalars) | declared diagnostic scalars | Static placeholder weights, never computed (no collectors exist); filtered from every operational path. Kept for registration/reporting only. **Mission-scope disposition (decided 2026-07-18, measured 127 = 82 metric-only + 45 operational SOFTWARE_ENGINEERING scalars):** code-quality telemetry is in scope only as *self-referential observability* — Mercury introspecting its own engineering health as part of the autonomous agent's self-maintenance mandate — and out of scope as a detection product (no external-codebase scoring service is planned). Consequences: (a) the 45 operational SW-eng scalars stay exactly where they are — they are part of the frozen σ_Immutable operational layout whose bit-stability *is* the config-integrity signal, and restructuring that layout is safety-forbidden (it collapsed the deterministic floor once); (b) the 82 metric-only scalars stay registered-dormant — reviving any of them requires a real collector plus the standard merit gate, and revival must never move them into the operational bands. |
