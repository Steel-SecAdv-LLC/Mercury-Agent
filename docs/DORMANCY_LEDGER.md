...
| `cognitive/ethical_bounding.py` `assess_weapons_uplift` | two-axis weapons/mass-casualty gate | Axis A is keyword routing; Axis B is regex intent matching + an *optional* pluggable classifier — a deterministic lexical heuristic, **not** a trained model. It is honest about this: fail-closed, CI-gated in both directions (`tests/ethical/test_weapons_uplift_gate.py`), and its coverage limits + compensating controls are stated plainly in [`HARM_POLICY.md`](HARM_POLICY.md) §8. Active safety control, not dormant — listed here so no caller mistakes the lexical matcher for an ML classifier. |
