...
| `core/global_omni_scalar_network.py` (~82 ISO-25010/Halstead/DORA/SLSA/… scalars) | declared diagnostic scalars | **21 of the 82 now carry REAL self-referential measurements; 61 stay documented placeholders.** `scripts/collect_sw_eng_metrics.py` computes real metrics on Mercury's own source tree — the Halstead suite (7), cyclomatic complexity (1) and Maintainability Index (3) via stdlib `ast`, plus the OpenSSF-Scorecard checks (10) from repo config — into `core/sw_eng_metrics.json`, overlaid at GOSNN init by `_apply_measured_sw_eng_metrics` (existing keys only; values kept in their penalty/positive direction band). The remaining 61 (the 31 ISO/IEC 25010 quality characteristics, the 10 NIST SAMATE + 4 ISO/IEC 5055 assurance measures, the 4 DORA CI/CD-telemetry metrics, the 4 NIST SSDF practices, the 4 SLSA levels, and the essential/design/cognitive/npath complexity variants) stay static placeholders — computing them would be fabrication or needs external process/telemetry data. Merit-gated by `tests/test_sw_eng_metrics_collector.py`. **Mission-scope disposition (decided 2026-07-18, measured 127 = 82 metric-only + 45 operational SOFTWARE_ENGINEERING scalars):** code-quality telemetry is in scope only as *self-referential observability* — Mercury introspecting its own engineering health as part of the autonomous agent's self-maintenance mandate — and out of scope as a detection product (no external-codebase scoring service is planned). Consequences: (a) the 45 operational SW-eng scalars stay exactly where they are — they are part of the frozen σ_Immutable operational layout whose bit-stability *is* the config-integrity signal, and restructuring that layout is safety-forbidden (it collapsed the deterministic floor once); (b) the metric-only scalars stay filtered — the 21 measured ones and the 61 placeholders alike are kept out of the operational bands (verified: the σ score is unchanged by the overlay), and reviving any placeholder still requires a real collector plus the standard merit gate. |
