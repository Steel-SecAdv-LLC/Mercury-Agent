...
| `core/global_omni_scalar_network.py` `MultiHeadAttentionFusion` | "32-head attention" learned fusion | **Training program exists; nothing ships — the production input is constant (measured 2026-07-14).** `scripts/train_gosnn_fusion.py` harvests the real `fuse()` inputs from a real engine detecting on real ADBench windows, but the harvest collapsed to **1 unique state list across 403 calls** (GOSNN registry groups are static between calls; the per-call base member is empty on the tabular detect path), and the fused output's only operational consumer is `1 + 0.1·mean(fused)` in `get_enhanced_scalars` — measured downstream effect on `anomaly_prob`: exactly 0. A reconstruction gate over a constant is vacuous (a memoriser "wins"), so the program's diversity tripwire refuses to train or ship (`artifacts/gosnn_fusion.eval.json`); `fuse()` keeps the deterministic phi-weighted reference, the historical random-projection inference remains unreachable, and the `load_shipped_weights` seam + artifact/package agreement are pinned by `tests/core/test_attention_fusion_gate.py`. Trainable for real once the registry carries per-call varying operational scalars. |
