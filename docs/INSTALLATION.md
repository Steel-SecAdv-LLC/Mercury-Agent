...
|---|---|---|
| `AMA_CRYPTO_VERSION` | `4.0.0` | Pinned PQC backend version; the import gate refuses a different *release* (`_pqc_gate._enforce_ama_version`). Matched PEP 440-tolerantly: `v4.0.0`, `4.0.0.post1`, `4.0` are accepted; `3.1.0` is not. |
...
v4.0.0`; `cmake -DAMA_USE_NATIVE_PQC=ON -DAMA_BUILD_SHARED=ON` + build;
...
(`v4.0.0`, `4.0.0.post1`, `4.0.0+cpu`, and even `4.0` are accepted; a different
