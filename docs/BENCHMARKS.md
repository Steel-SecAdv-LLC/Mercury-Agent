...
> historical **CI regression-gate floor** (the gate trips ~7% below
> it: ROC-AUC 0.75 / F1 0.55), preserved here for the auditable
...
## Transductive ADBench — base vs current (per-set, committed)

The `14 W / 2 tie / 2 L` headline is backed by two committed harness runs:
`research/omni_equation/adbench_base_e118e1f.json` (base, pre-hardening) and
`research/omni_equation/adbench_results.json` (hardened head). The per-set diff
is committed at `research/omni_equation/adbench_base_vs_current.json` and
regenerated by the harness's own `--baseline` flag (the diff logic lives in the
engine-free `research/omni_equation/_compare.py`, pinned by
`tests/research/test_adbench_baseline_compare.py`):

```bash
python research/omni_equation/harness_adbench.py \
    --baseline research/omni_equation/adbench_base_e118e1f.json
```

| Dataset | base | hardened | Δ AUROC | verdict | data-type gate |
|---|---|---|---|---|---|
| glass | 0.6461 | 0.7442 | +0.0981 | win | temporal→tabular |
| Stamps | 0.7952 | 0.8702 | +0.0750 | win | temporal→tabular |
| PageBlocks | 0.8604 | 0.9160 | +0.0556 | win | temporal→tabular |
| wine | 0.6429 | 0.6824 | +0.0395 | win | temporal→tabular |
| Ionosphere | 0.8753 | 0.9116 | +0.0363 | win | temporal→tabular |
| mammography | 0.8354 | 0.8664 | +0.0310 | win | temporal→tabular |
| thyroid | 0.9079 | 0.9385 | +0.0306 | win | temporal→tabular |
| Pima | 0.6441 | 0.6718 | +0.0277 | win | temporal→tabular |
| breastw | 0.9551 | 0.9715 | +0.0164 | win | temporal→tabular |
| optdigits | 0.4972 | 0.5043 | +0.0071 | win | tabular |
| Cardiotocography | 0.6248 | 0.6288 | +0.0040 | win | tabular |
| cardio | 0.9060 | 0.9084 | +0.0024 | win | tabular |
| vertebral | 0.3998 | 0.4013 | +0.0015 | win | temporal→tabular |
| WBC | 0.9869 | 0.9878 | +0.0009 | win | tabular |
| Hepatitis | 0.6820 | 0.6820 | +0.0000 | **break-even** | tabular |
| Lymphography | 0.9918 | 0.9918 | +0.0000 | **break-even** | tabular |
| WPBC | 0.4770 | 0.4768 | −0.0002 | **loss** | tabular |
| Waveform | 0.5870 | 0.5867 | −0.0003 | **loss** | temporal→tabular |

**Mean AUROC 0.7397 → 0.7634 (+0.0237); 14 W / 2 tie / 2 L.** The 10 largest
wins are the data-type gate fix landing (base mislabeled these `TEMPORAL`,
applying the order-dependent residual filter and over-weighting kinematic);
the four small wins (optdigits, Cardiotocography, cardio, WBC) were already
`tabular` and move only on margin sharpening. The two break-evens are
already-tabular sets whose ranking neither lever changes. The two "losses" are
≤0.0003 (numerical noise) — the dilution gap documented in
`research/omni_equation/FINDINGS.md`, not a capability regression.
...
| Scalability | 0.937 | Empirical complexity slope d log(t) / d log(n) = 1.063 over N ∈ [200, 800, 3200]. |
