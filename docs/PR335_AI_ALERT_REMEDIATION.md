...
|---|---|---|---|---|---|
| A | `scripts/verify_data_credentials.py` · `check_*` | Transport failure reported as the opaque `"HTTP 0"` (`r3566583308`) | `_get()` returns `(0, <exc text>)` on DNS/TLS/timeout; callers rendered only `f"HTTP {status}"`, discarding `body` (the exception) | New `_fail_detail(status, body)` surfaces the transport error text when `status==0` and a short provider-body preview on a real HTTP error; all five keyed checkers route their non-200 path through it | `tests/test_data_credentials_offline.py` (28 cases) |
| A2 | `scripts/verify_data_credentials.py` · `_fail_detail` | **Fix A could leak the API key** — a `requests` transport error embeds the request URL, so surfacing it raw prints `?api_key=<secret>` to CI logs (`r3567002515`, raised on the remediation commit) | The "surface the transport error" fix (A) reintroduced a secret-leak the module explicitly forbids | New `_redact()` replaces the value of any credential query parameter (`api_key`/`apikey`/`appid`/`token`/`password`/`secret`/`key`) with `***`; `_fail_detail` scrubs its text through it before returning | `TestRedaction` + `test_checkers_never_leak_key_on_transport_error` (13 cases) |
| A3 | `scripts/verify_data_credentials.py` · `run` | Docstring said "returns the failure count as exit code" but the code returns `1` on any failure (`r3567002519`) | The docstring added in Fix D overstated the contract | Docstring corrected to "exit 1 if any present key fails, else 0" (matches the unchanged, intended behaviour) | Covered by the `run()` smoke path |
