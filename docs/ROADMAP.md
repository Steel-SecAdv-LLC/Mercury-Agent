...
| 19 | Orphaned modules: `core/di.py` + `detectors/cross_domain_frequency.py` wire-or-retire | **OPEN (decision needed).** The 2026-07-21 coverage round found both modules have **zero importers** anywhere in src/tests/scripts/tools (`core.di`'s previously-claimed importers were regex false positives; `cross_domain_frequency` is referenced only in docs — the `CrossDomainCorrelation` in `core/learnable_gosnn.py` is an unrelated same-named `nn.Module`). `di.py`'s detector/model maps had bit-rotted to nonexistent class names (fixed in the same round — only possible in dead code), and its constructor injection cannot work for any class defined under `from __future__ import annotations` (string annotations → silently no injection), which is all of src/. Either wire these surfaces into the runtime or retire them through DEPRECATION.md; leaving them orphaned re-invites bit rot. | `tests/core/test_di.py` (56 tests, 94%) + `tests/detectors/test_cross_domain_frequency.py` (31 tests, 98%) pin both contracts so they cannot rot further while the decision is pending; `docs/MAINT1_2_COVERAGE_REMEDIATION.md` §8 records the evidence. |
