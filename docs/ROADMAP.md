...
| 3 (closed) | VLM detector surface | **CLOSED (2026-06-02).** `detectors/vlm/base_vlm.py`'s five contract methods (`_initialize_model`, `_create_prompt`, `_parse_response`, `detect`, `extract_features`) are now genuine `@abstractmethod` declarations — no `NotImplementedError` stub on the public path; `BaseVLMDetector()` raises `TypeError`. The surface is explicitly marked **experimental** (per the 2026-05 strategic decision to ship native detectors, not BLIP/GPT adapters). | `tests/test_vlm_detectors.py::TestBaseVLMDetector` — base is abstract / not instantiable, the five methods are abstract, and a concrete subclass exercises the concrete `_sample_frames` helper. |
...
> | VLM detectors | ✓ | experimental | — | **Honest ABC (2026-06-02).** `detectors/vlm/base_vlm.py`'s five contract methods are `@abstractmethod` (no `NotImplementedError` on the public path; direct instantiation raises `TypeError`). Marked **experimental** per the 2026-05 strategic decision to ship native detectors, not BLIP/GPT adapters. Locked by `tests/test_vlm_detectors.py::TestBaseVLMDetector`. |
