...
| 16 (closed) | Fusion checkpoint round-trip fidelity + shipped-checkpoint quality | **CLOSED (2026-06-11).** Checkpoint format **v2** executes the remediation plan and removes the root causes. (1) `save_model` embeds the exact array `fit_fusion` fit the base detectors on (`detector_refit_reference`, bounded by `DETECTOR_REFIT_REFERENCE_MAX_BYTES`); `load_model` refits the detectors on it — every base-detector fit is a deterministic function of its data — so a loaded engine's serve-path features match training time and the first inference batch is never leaked into an auto-fit. (2) The fitted conformal classifier round-trips (`BinaryConformalClassifier.export_state`/`from_state`) and the restored temperature is the same numpy scalar type `fit()` produces (a python `float` weak-promoted float32 logits and wobbled tail probabilities ~1e-7). (3) Five feature-path impurities that made checkpoints unreproducible are fixed at the source: fixed-seed init for the untrained `temporal` LSTM and `dimensional` autoencoder projectors; content-seeded (input-pure) draws in the placeholder `affective` model; fixed-seed initial universes in `multiverse`; non-committing memory for `neural`/`directive` feature extraction (accrual stays on the `predict`/`detect` paths). Measured: `max\|Δ_prob\|` = **0.0** on the regenerated `default_fusion.pt` (was ≈0.76); a freshly-loaded engine reports held-out AUC 1.0000 on the training distribution (was ≈0.70 via the auto-fit leak). v1 checkpoints still load with the documented degraded path. | `tests/test_fusion_checkpoint_roundtrip.py` (8 tests: probability equivalence < 1e-3, conformal/temperature round-trip + stale-state resets, v1 back-compat, reference size guard, rescore bit-stability, feature instance-independence) + the regeneration self-check in `scripts/train_default_fusion.py`. |
