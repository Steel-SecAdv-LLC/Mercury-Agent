...
def main() -> int:
    ...
    if args.source == "synthetic":
        # Round-trip fidelity: the persisted detector/model fitted state must
        # leave the fresh engine's detectors fitted before any inference (no
        # first-batch auto-fit leak), and its per-sample probabilities must
        # match the training engine's (ROADMAP row 16; locked in CI by
        # tests/test_fusion_checkpoint_roundtrip.py).
        assert all(
            det.is_fitted() for det in fresh.detectors.values()
        ), "detectors should be restored as fitted from the checkpoint on load"
