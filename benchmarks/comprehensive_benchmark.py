...
def run_all_benchmarks(include_real_data: bool = True) -> dict[str, Any]:
    ...
    print(f"  ✓ Runtime: {benchmarks['space_exploration']['runtime_ms']:.2f} ms")
    print(f"  ✓ Anomaly detected: {benchmarks['space_exploration']['anomaly_detected']}")

    print(f"\n[3/{total_steps}] Benchmarking simulation module...")
    benchmarks["simulation"] = benchmark_simulation_module()
    print(f"  ✓ Collatz: {benchmarks['simulation']['collatz_exploration_ms']:.2f} ms")
    print(f"  ✓ Millennium: {benchmarks['simulation']['millennium_analysis_ms']:.2f} ms")
    ...
    if include_real_data:
        ...
        print(f"  ✓ F1 Score: {benchmarks['nsl_kdd']['f1']:.4f}")
        print(f"  ✓ ROC-AUC: {benchmarks['nsl_kdd']['roc_auc']:.4f}")
        print(f"  ✓ Data Source: {benchmarks['nsl_kdd']['data_source']}")

        print(f"\n[6/{total_steps}] Benchmarking MIMIC-III Demo (medical domain)...")
        benchmarks["mimic_demo"] = benchmark_real_data_mimic()
        print(f"  ✓ F1 Score: {benchmarks['mimic_demo']['f1']:.4f}")
        print(f"  ✓ ROC-AUC: {benchmarks['mimic_demo']['roc_auc']:.4f}")
        print(f"  ✓ Data Source: {benchmarks['mimic_demo']['data_source']}")
    ...
    for benchmark_name, benchmark_result in benchmarks.items():
        validate_metrics(benchmark_result, benchmark_name)
    print("  ✓ All metrics within valid range [0, 1]")

    results: dict[str, Any] = {"timestamp": time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime()), "benchmarks": benchmarks}
