...
DATASETS = ["6_cardio", "23_mammography", "38_thyroid", "31_satimage-2",
...
def run_track(small: bool):
    ...
    return dict(acc=acc, iso_loses=iso_loses_auroc, beta_pres=beta_preserves,
                platt_pres=platt_preserves, temp_pres=temp_preserves,
                ens4_pick=ens4_pick, ece_best=per_ds_ece_best, brier=per_ds_brier)
...
def report(track_name, R):
    ...
    for m in methods:
        a = R["acc"][m]
        print(f"{m:12s} {fmt(a['auroc']):>16s} {fmt(a['brier']):>16s} "
    ...
    print(f"Isotonic loses AUROC vs raw on {n_iso_loses}/6 datasets "
          f"(per-ds frac: {[round(iso_all[ds],2) for ds in DATASETS]})")
    print(f"Beta preserves AUROC exactly: {np.mean(R['beta_pres'])*100:.0f}% of runs; "
