...
def test_name_resolves_to_dataset(config_name, expected_name, expected_index, tmp_path):
...
def test_explicit_preprocessing_overrides_name(tmp_path):
...
def test_registry_creates_distinct_loaders(tmp_path):
...
def test_check_data_exists_is_file_specific(tmp_path):
