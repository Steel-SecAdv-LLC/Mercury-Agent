...
"""Fusion and decorrelator tests for the Anomaly Math Revolver."""
...
from omni_mercury_engine.detectors.math_revolver.base_probe import ProbeResult
...
class TestNoRedundancyLeavesWeightsUnchanged:
    ...
    def test_no_redundancy_leaves_weights_unchanged(self) -> None:
        ...
        score_matrix = np.column_stack([col_a, col_b, col_c]).astype(
            np.float64
        )
...
class TestFailOpenWhenDecorrelatorNotCalibrated:
    ...
    def test_fail_open_when_decorrelator_not_calibrated(self) -> None:
        ...
        results = [
            _make_probe_result(
                "probe_a", rng.uniform(0, 1, n_samples).astype(np.float64)
            ),
            _make_probe_result(
                "probe_b", rng.uniform(0, 1, n_samples).astype(np.float64)
            ),
        ]
...
# ===================================================================
# Revolver: auto-calibrate on fit
# ===================================================================
