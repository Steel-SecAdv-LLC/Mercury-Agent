...
def test_detect_per_probe_shape() -> None:
    ...
    assert np.all(matrix >= 0.0) and np.all(matrix <= 1.0), "All scores must be in [0, 1]"
...
def test_ama_cv_weights_shift_on_degraded_mercury() -> None:
    ...
    assert det._ama_weight >= 0.15, (
        f"AMA weight {det._ama_weight:.3f} too low for data where " f"Mercury components struggle"
    )
