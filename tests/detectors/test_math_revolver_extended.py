...
"""Extended tests for Anomaly Math Revolver — probes 9-21."""
...
class TestQuantumAnnealingProbe:
    ...
    def test_detects_thermodynamic_outlier(self) -> None:
        ...
        result = probe.deviation_score(test_data)
        assert result.deviation_scores[100] > float(
            np.median(result.deviation_scores[:80])
        )
...
class TestBoltzmannCouplingProbe:
    ...
    def test_detects_coupling_structure_break(self) -> None:
        ...
        coupled_normal = (np.sin(t) + 0.3 * np.sin(2 * t + 0.5)).astype(
            np.float64
        )
        ...
        coupled_broken[250:] = (
            np.sin(t[250:]) + 0.3 * np.sin(5 * t[250:] + 2.0)
        )
...
# ===================================================================
# Full 21-probe Revolver integration
# ===================================================================


class TestFullRevolver21:
    """Integration tests for the full 21-probe revolver."""
    ...
    def test_all_21_probes_fit(self) -> None:
        ...
        revolver.fit(data)
        assert revolver.active_probe_count == 21
    ...
    def test_degraded_ensemble_still_detects(self) -> None:
        ...
        for i in range(0, 21, 2):
            revolver._probes[i]._is_fitted = False
    ...
    def test_domain_affinity_all_21_probes(self) -> None:
        for domain in [
            ...
            revolver = AnomalyMathRevolver(domain=domain)
            data = make_normal_signal(200)
            revolver.fit(data)
            scores = revolver.detect(data)
    ...
    def test_decorrelator_auto_calibrates_on_fit(self) -> None:
        ...
        data = make_normal_signal(200)
        revolver.fit(data)
        assert revolver._decorrelator.is_calibrated

    def test_get_correlation_report_structure(self) -> None:
        revolver = AnomalyMathRevolver()
        data = make_normal_signal(200)
        revolver.fit(data)
        report = revolver.get_correlation_report()
