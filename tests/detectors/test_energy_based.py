...
"""Tests for the EnergyBasedDetector regime-novelty detector."""
...
class TestContract:
    ...
    def test_scores_shape_and_range(self) -> None:
        ...
        scores = np.asarray(EnergyBasedDetector().fit(x).detect(x)["scores"])
        assert scores.shape == (len(x),)
        assert float(scores.min()) >= 0.0 and float(scores.max()) <= 1.0

    def test_extract_features_shape(self) -> None:
        x = _series_with_segment()
        feats = EnergyBasedDetector().fit(x).extract_features(x)
        assert feats.shape == (len(x), 1)
        assert np.isfinite(feats).all()

    def test_invalid_params(self) -> None:
        with pytest.raises(ValueError):
            EnergyBasedDetector(embed_dim=0)
        with pytest.raises(ValueError):
            EnergyBasedDetector(ridge=0.0)

    def test_deterministic(self) -> None:
        x = _series_with_segment(7)
        a = EnergyBasedDetector().fit(x).detect(x)["scores"]
        b = EnergyBasedDetector().fit(x).detect(x)["scores"]
...
class TestSignal:
    def test_segment_flagged(self) -> None:
        ...
        scores = np.asarray(EnergyBasedDetector().fit(train).detect(x)["scores"])
        seg = scores[400:500]
    ...
    def test_low_false_positive_rate(self) -> None:
        ...
        scores = np.asarray(
            EnergyBasedDetector(calibration_quantile=0.98).fit(train).detect(test)["scores"]
        )
