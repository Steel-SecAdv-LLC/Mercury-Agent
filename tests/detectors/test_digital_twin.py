...
"""Tests for the digital-twin (AR forward-model) residual detector."""
...
class TestContract:
    ...
    def test_scores_shape_and_range(self) -> None:
        ...
        scores = np.asarray(DigitalTwinResidualDetector().fit(x).detect(x)["scores"])
        assert scores.shape == (len(x),)
        assert float(scores.min()) >= 0.0 and float(scores.max()) <= 1.0

    def test_extract_features_shape(self) -> None:
        x = _series_with_spike()
        feats = DigitalTwinResidualDetector().fit(x).extract_features(x)
        assert feats.shape == (len(x), 1)
        assert np.isfinite(feats).all()

    def test_invalid_params(self) -> None:
        with pytest.raises(ValueError):
            DigitalTwinResidualDetector(order=0)
        with pytest.raises(ValueError):
            DigitalTwinResidualDetector(horizon=0)

    def test_deterministic(self) -> None:
        x = _series_with_spike(7)
        a = DigitalTwinResidualDetector().fit(x).detect(x)["scores"]
        b = DigitalTwinResidualDetector().fit(x).detect(x)["scores"]
...
class TestSignal:
    def test_spike_flagged(self) -> None:
        ...
        scores = np.asarray(DigitalTwinResidualDetector().fit(train).detect(x)["scores"])
    ...
    def test_low_false_positive_rate(self) -> None:
        ...
        scores = np.asarray(
            DigitalTwinResidualDetector(calibration_quantile=0.98).fit(train).detect(test)["scores"]
        )
